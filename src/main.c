...
/*   Updated: 2023/04/19 09:27:50 by francoma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			for(k = 0; k < 3 && !t; ++k)
// 			{
// 				const FPR t2 = ray_intersect(ray, ctx->scene.objs[k]);
// 				if (t2 > 0.0 && (t == 0.0 || t2 < t))
// 				{
// 					t = t2;
// 					hit = vec3_add(ray.pos, vec3_scale(ray.dir, t));
// 					n = vec3_normalize(vec3_sub(hit, ctx->scene.objs[k]->sphere.pos));
// 					sphere = &ctx->scene.objs[k]->sphere;
// 				}
// 			}
// 			if (t)
// 			{
// 				const t_vec3	light_dir = vec3_normalize(vec3_sub(ctx->scene.lights[0]->pos, hit));
// 				const FPR		diffuse = fmax(0.0, vec3_dot(light_dir, n));
// 				const t_vec3	diffuse_color = vec3_mult(vec3_scale(ctx->scene.lights[0]->color, diffuse), sphere->color);
// 				const t_vec3	colorv = vec3_add(vec3_scale(diffuse_color, 0.7), ctx->scene.a_light.color);
